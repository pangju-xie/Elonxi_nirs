...
/*****************************************************************************
  ...
  * 		 void
*****************************************************************************/
int packetSendData(uint8_t *data, uint8_t *src, uint16_t length, uint32_t stamp)
{
    uint16_t i=0;
    uint16_t crc=0;
...
int udpSendSensorData(uint8_t type)
{
	...
	int len=0;
    //uint16_t len_data = SAMPLE_BYTES*SAMPLE_CHANNEL*g_app_var.emg_dr_pack;
    uint16_t len_data = NIRS_BYTES*NIRS_CHANNEL*g_app_var.nirs_dr_pack;

    //memset(sDb.payload, 0, 1024*25);
    if(type == 0){
        len = packetSendMessage(g_app_var.payload,g_app_var.serialNumber,g_app_var.synFlag);
    ...
    {
        //for sensor data 
        len = packetSendData(g_app_var.payload, G_nirs_data.SendData, len_data, g_app_var.packet_counter);

        //memcpy(&g_struct_para.sd_buffer[SD_BASE_LEN*g_app_var.sd_count],g_struct_para.send_data,SD_BASE_LEN);
        memcpy(&G_nirs_data.sd_buffer[NIRS_DATA_LEN*g_app_var.sd_count], g_app_var.payload, len);
        g_app_var.sd_count++;   

        if(g_app_var.sd_count >= NIRS_SD_INT)
        {
            g_app_var.sd_count = 0;
            g_app_var.sd_ready_packcnt = g_app_var.packet_counter - NIRS_SD_INT + 1;
            g_app_var.sd_write_flag = 1;
        }

        #if 0
        if(g_app_var.rf_syn_packcnt > 0)
        {
            if((g_app_var.packet_counter > g_app_var.rf_syn_packcnt) || (g_app_var.packet_counter < g_app_var.rf_syn_packcnt))
            {
                printf("[%ld syn = %ld]\r\n",g_app_var.packet_counter,g_app_var.rf_syn_packcnt);
                g_app_var.packet_counter = g_app_var.rf_syn_packcnt - 1;
