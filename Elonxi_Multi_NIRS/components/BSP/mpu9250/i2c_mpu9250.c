...
static const bool REQUIRE_CALIBRATION               = false;  //true;
...
extern void app_imu_send_task(void);

uint32_t g_imu_packcnt = 0;
uint8_t  g_imu_send_flag = 0;
uint8_t  g_isIMU = 0;
...
void mpu9250_task(void *calibration) {
    ...
    while (1) {
        if(g_isIMU)
        {
            ...
            // ESP_LOGI(TAG, "Accel: X=%d, Y=%d, Z=%d    |   Gyro: X=%d, Y=%d, Z=%d   |   Heading=%d, Pitch=%d, Roll=%d\n",
            //     accel_x, accel_y, accel_z, 
            //     gyro_x, gyro_y, gyro_z, 
            //     mag_x, mag_y, mag_z);
            
            g_struct_imu_para.imu_data[0+cnt*18] = (accel_x >> 8) & 0xFF;
            g_struct_imu_para.imu_data[1+cnt*18] = accel_x & 0xFF;
            g_struct_imu_para.imu_data[2+cnt*18] = (accel_y >> 8) & 0xFF;
            g_struct_imu_para.imu_data[3+cnt*18] = accel_y & 0xFF;
            g_struct_imu_para.imu_data[4+cnt*18] = (accel_z >> 8) & 0xFF;
            g_struct_imu_para.imu_data[5+cnt*18] = accel_z & 0xFF;

            g_struct_imu_para.imu_data[6+cnt*18] = (gyro_x >> 8) & 0xFF;
            g_struct_imu_para.imu_data[7+cnt*18] = gyro_x & 0xFF;
            g_struct_imu_para.imu_data[8+cnt*18] = (gyro_y >> 8) & 0xFF;
            g_struct_imu_para.imu_data[9+cnt*18] = gyro_y & 0xFF;
            g_struct_imu_para.imu_data[10+cnt*18] = (gyro_z >> 8) & 0xFF;
            g_struct_imu_para.imu_data[11+cnt*18] = gyro_z & 0xFF;

            g_struct_imu_para.imu_data[12+cnt*18] = (mag_x >> 8) & 0xFF;
            g_struct_imu_para.imu_data[13+cnt*18] = mag_x & 0xFF;
            g_struct_imu_para.imu_data[14+cnt*18] = (mag_y >> 8) & 0xFF;
            g_struct_imu_para.imu_data[15+cnt*18] = mag_y & 0xFF;
            g_struct_imu_para.imu_data[16+cnt*18] = (mag_z >> 8) & 0xFF;
            g_struct_imu_para.imu_data[17+cnt*18] = mag_z & 0xFF;
...
void mpu9250_init(void) {
    ...
    if (REQUIRE_CALIBRATION){
        ...
        ESP_LOGE(TAG, "Skipping Calibration");
    }
}
