...
//            g_nNextRFState = RF_STATE_TX_TIMEOUT;
		if( (INFINITE != g_nTxTimeout) && ((g_nTxTimeCount) > 0xffff) )
		g_nNextRFState = RF_STATE_TX_TIMEOUT;
...
//379~510
int Cmt2300aSetTxFreq(uint32_t freq)
{
	int sta = -1;
	uint8_t divx_code = 1;
	uint8_t divder = 4;
	...
	nk = (double)freq/1000000*divder/26;
	
	n = (uint8_t)(nk/1);
	k = (uint32_t)((nk - (uint32_t)n)*1000000);
	
	for(uint8_t i = 0; i < 8; i++)
	{
		reg[i] = CMT2300A_ReadReg(i+0x18);
	}
	
	reg[3] &= (~(1<<7));
	reg[3] &= (~(7<<4));
	
	reg[3] |= ( ( pa_ldo << 7 ) | (divx_code<<4) );
	...
	reg[7] |= ( (vco_bank<<4) | ((k&0xe0000)>>16) );
	
	CMT2300A_ConfigRegBank(CMT2300A_FREQUENCY_BANK_ADDR , reg , CMT2300A_FREQUENCY_BANK_SIZE );
