 ...
 */
esp_err_t i2c_register_read(i2c_obj_t *self, uint16_t addr, const uint8_t reg, uint8_t *data, const size_t len)
{
    ...
    i2c_transfer(self, addr, 2, bufs,I2C_FLAG_WRITE | I2C_FLAG_READ | I2C_FLAG_STOP);
    return ESP_OK;
}
 ...
 * @retval      错误值        ：0成功，其他值：错误
 */
esp_err_t i2c_register_write_byte(i2c_obj_t *self, uint16_t addr, uint8_t reg, uint8_t data)
{
    uint8_t memaddr_buf[1];
    memaddr_buf[0]  = reg;
    ...
    i2c_transfer(self, addr, 2, bufs,I2C_FLAG_STOP);
    return ESP_OK;
}
